...
- **結構化知識庫問答服務 (`query_service.py`)**：
  - **資料來源分工體系**：
    - 🎓 **歷屆論文索引庫** (`thesis_index.json`)：維持結構化 JSON，收錄歷屆論文題目、英文題目、關鍵字與作者，支援題目錯別字/同音字與主題關鍵字之毫秒級極速秒查。
    - 👥 **歷屆校友資料庫** (`lab_alumni.json`)：維持結構化 JSON，姓名、系所、專長領域與畢業年份查詢。
    - 🖥️ **設備財產清冊** (`lab_properties.json`)：維持結構化 JSON，財產編號、物品名稱、保管人與存放位置。
    - 💰 **公用經費與計畫帳本**：**改為直接連線 MediaWiki 知識庫 (`wiki_reader.py`) 檢索**（取得線上「公用經費清單」頁面與 Google 試算表 `1un-VyqNpeIMiQvHZSW51Koo7_WZI-XwQdxs_mTc7O2w/#gid=9`，並整合線上報帳與經費規章，本地帳本 `lab_funds_ledger.json` 作為結構化補充明細）。
    - 📜 **會計報銷法規** (`accounting_rules.json`)：元智大學各類費用報銷標準與規定檢索。
  - **多關鍵字複合分詞檢索 (Multi-keyword Token Search)**：校友、論文、財產、帳本、法規全面支援空格分詞 (AND) 複合查詢。
  - **360 度人物/專案全景速查 (`query_person_360`)**：一鍵聯查成員之學歷年份、畢業論文題目與簡報、保管設備清冊與經費收支來源。
...
- **MediaWiki 實驗室維基知識庫 (`wiki_reader.py`)**：
  ...
  - **12 小時快取自動過期同步機制 (Cache TTL Auto-Sync)**：知識庫查詢引擎內建時間戳檢驗，當本地資料庫超過 12 小時未更新時，系統在執行問答時會自動背景連線線上資料源進行同步，兼顧極速秒回與資料新鮮度。
...
- **效能指標與基準測試規範 (Performance & Benchmark Specifications)**：
  - **本地單一資料庫檢索**：校友、設備、經費、法規與合規試算查詢回應時間需維持在 `< 1 ms`。
  - **模糊比對與多分詞搜尋**：論文檢索暖機後需維持在 `< 10 ms`。
  - **跨庫聯查 (360° 全景速查)**：需在 `< 15 ms` 內整合完畢。
